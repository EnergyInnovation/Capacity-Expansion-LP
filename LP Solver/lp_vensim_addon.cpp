// lp_vensim_addon.cpp — HiGHS C API (multi-scenario, post-installed fix)
//
// Highlights:
...
//  • FIX: LP_REV_EN_PERMW_INSTALLED now divides by Installed_post = Existing + Build − Retire
//
  ...
  if(g_lastCode==0){
    ...
    if(Highs_getSolution(h,col_val.data(),col_dual.data(),row_val.data(),row_dual.data())==0){
      ...
      for(int r=0;r<R;++r){
		...
		// Use ExistingCap directly so we aren't sensitive to any cleared snapshot.
  ...
  int* num_literal, int* num_lookup)
{
  ...
  *dim_act=0; *modify=0; *num_loops=0; *num_literal=0; *num_lookup=0;
  ...
  switch(setup_index){
    case 0:
      *sym=(char*)"LP_Solve";
      // Vectors (20): Demand, CF, Hours, PeakMask, CAPEX, FOM_new, Fuel, VOM,
      //               ExistingCap, MaxBuild, CapMax, TransRR,
      //               CES_q, RPS_q, FOM_exist, RetireCost, RevExist, RetireResponse, BelowFOMFlag
      // Scalars: CES_rhs, CES_ACP, RPS_rhs, RPS_ACP, FOM_cover_mult, ReserveMargin, R,S,H,E, scenario_id
      *arglist=(char*)"{Demand},{Capacity_factor},{Hours},{PeakMask},{CAPEX},{FOM_new},{Fuel},{VOM},{ExistingCap},{MaxBuild},{CapMax},{TransRR},{CES_qualifying},{RPS_qualifying},{FOM_exist},{RetireCost},{RevExist},{RetireResponse},{BelowFOMFlag},CES_rhs,CES_ACP,RPS_rhs,RPS_ACP,FOM_cover_mult,ReserveMargin,R,S,H,E,scenario_id";
      *num_args=30; *num_vector=20; *func_index=F_SOLVE; return 1;
...
extern "C" __declspec(dllexport) int VEFCC vensim_external(VV* val, int nval, int funcid){
  ...
  switch(funcid){
    ...
    case F_SOLVE: {
      if(nval<30){ val[0].val=1e308; return 0; }
      ...
      double CES_rhs = val[19].val, CES_ACP = val[20].val;
      double RPS_rhs = val[21].val, RPS_ACP = val[22].val;
      double FOM_cover_mult = val[23].val;
      double ReserveMargin  = val[24].val;

      int R=(int)(val[25].val+0.5), S=(int)(val[26].val+0.5), H=(int)(val[27].val+0.5), E=(int)(val[28].val+0.5);
      int scenario_id = (nval>=30)? (int)(val[29].val+0.5) : 0;
